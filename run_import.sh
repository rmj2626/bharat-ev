...
echo "Converting CSV file for PostgreSQL COPY..."
# Prepare the CSV data by removing the header and storing in a temp file
TEMP_CSV="vehicles_temp.csv"
tail -n +2 attached_assets/vehicles_final.csv > $TEMP_CSV

echo "Counting current vehicles..."
...
echo "Copying data from CSV file..."
# Copy data from the CSV file into the temporary table
psql $DATABASE_URL -c "\COPY temp_vehicles FROM '$TEMP_CSV' WITH CSV;"

echo "Inserting vehicles data..."
